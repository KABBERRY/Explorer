...
const api = `${process.env.config.api.host}:${process.env.config.api.portWorker}${process.env.config.api.prefix}`; //@todo remove DefinePlugin from webpack and use the public config (as we now have config.server.js)
...
// Request the list of connected peers.
const getAddnodes = () => fetch(`${api}/addnodes`);
...
// Request Social Aggregation
const getSocial = query => fetch(`${api}/social`, query);
...
// Broadcasts raw tx
const sendrawtransaction = query => fetch(`${api}/sendrawtransaction`, query);

// Off-Chain Sign On
const login = query => fetch(`${api}/login`, null, query, 'POST');
